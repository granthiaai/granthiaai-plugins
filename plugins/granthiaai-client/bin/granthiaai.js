...
var userRole = external_exports.enum(["owner", "admin", "member"]);
var tenantStatus = external_exports.enum(["trialing", "active", "free", "suspended", "closed"]);
var embeddingStatus = external_exports.enum(["pending", "embedded", "failed"]);
var currency = external_exports.enum(["USD", "INR"]);

// ../shared/dist/ingest.js
var ingestChunk = external_exports.object({
  turn_index: external_exports.number().int().nonnegative(),
  user_content: external_exports.string(),
  assistant_content: external_exports.string(),
  // ISO 8601 instant; offsets accepted, normalized to UTC by the engine and
  // clamped against absurd future values before becoming created_at.
  timestamp: external_exports.string().datetime({ offset: true })
});
var ingestRequest = external_exports.object({
  repo_url: external_exports.string().min(1),
  hostname: external_exports.string().min(1),
  session_id: external_exports.string().min(1),
  chunks: external_exports.array(ingestChunk).min(1)
});
var ingestResponse = external_exports.object({
  synced: external_exports.number().int().nonnegative(),
  // Optional advisory floor (spec §10): the minimum supported client version.
...
// ../shared/dist/search.js
var DEFAULT_SEARCH_LIMIT = 5;
var MAX_SEARCH_LIMIT = 20;
var CANDIDATE_MULTIPLIER = 10;
var HNSW_EF_SEARCH = 100;
var RESTRICTED_CANDIDATE_MULTIPLIER = CANDIDATE_MULTIPLIER * 4;
var RESTRICTED_HNSW_EF_SEARCH = HNSW_EF_SEARCH * 4;
...
// ../shared/dist/control.js
var region = external_exports.enum(["us", "eu", "in", "ap"]);
var accessMode = external_exports.enum(["all", "restricted"]);
var onboardingRequest = external_exports.object({
  ...
  billingCountry: external_exports.string().regex(/^[A-Z]{2}$/),
  requestedRegion: region,
  acceptedTermsVersion: external_exports.string().min(1),
  acceptedPrivacyVersion: external_exports.string().min(1)
});
var inviteRequest = external_exports.object({
  email: external_exports.string().email(),
  role: userRole,
  accessMode
});
var inviteAccept = external_exports.object({
  token: external_exports.string().min(1),
  acceptedTermsVersion: external_exports.string().min(1),
  acceptedPrivacyVersion: external_exports.string().min(1)
});
var memberSummary = external_exports.object({
  userId: external_exports.string().min(1),
  email: external_exports.string().email(),
  role: userRole,
  joinedAt: external_exports.string().min(1)
});
var pendingInvite = external_exports.object({
...
var roleChangeRequest = external_exports.object({
  role: userRole
});
var usageSummary = external_exports.object({
  projectCount: external_exports.number().int().nonnegative(),
  chunkCount: external_exports.number().int().nonnegative(),
  // null when the tenant has never synced
...
var meResponse = external_exports.object({
  ...
  tenantStatus,
  region,
  dataPlaneBaseUrl: external_exports.string().url(),
  placementRegion: region,
  requestedRegion: region
});
var excludedProjectsPolicy = external_exports.object({
  repoUrls: external_exports.array(external_exports.string())
});
var auditLogEntry = external_exports.object({
  id: external_exports.string(),
  actor: external_exports.string(),
  action: external_exports.string(),
  targetType: external_exports.string().nullable(),
  targetId: external_exports.string().nullable(),
  metadata: external_exports.record(external_exports.unknown()),
  ip: external_exports.string().nullable(),
  createdAt: external_exports.string().datetime()
});
var deletionRequest = external_exports.object({
  projectId: external_exports.string().min(1).optional(),
  developerEmail: external_exports.string().email().optional()
}).refine((data) => {
  const hasProject = data.projectId !== void 0;
  const hasEmail = data.developerEmail !== void 0;
  return hasProject !== hasEmail;
}, {
  message: "Exactly one of projectId or developerEmail must be provided, not both and not neither."
});
var accessModeRequest = external_exports.object({ accessMode });
var projectAccessDecisionRequest = external_exports.object({
  effect: external_exports.enum(["allow", "deny"])
});
var accessReason = external_exports.enum(["assigned", "contributed", "blocked", "default", "unrestricted"]);
var memberProjectAccess = external_exports.object({
...
var memberAccessResponse = external_exports.object({
  accessMode,
  projects: external_exports.array(memberProjectAccess)
});
var projectMemberAccess = external_exports.object({
  userId: external_exports.string().uuid(),
  email: external_exports.string().email(),
  visible: external_exports.boolean(),
  reason: accessReason
});
var projectAccessMirrorResponse = external_exports.object({
  projectId: external_exports.string().uuid(),
  repoUrl: external_exports.string().min(1),
  displayName: external_exports.string().nullable(),
  members: external_exports.array(projectMemberAccess)
});

// ../shared/dist/metering.js
var usageMeters = external_exports.object({
  storage: external_exports.object({
    textBytes: external_exports.number().nonnegative(),
    vectorBytes: external_exports.number().nonnegative(),
    indexBytes: external_exports.number().nonnegative(),
    totalBytes: external_exports.number().nonnegative(),
    gb: external_exports.number().nonnegative()
  }),
  searches: external_exports.object({
    count: external_exports.number().int().nonnegative(),
    periodStart: external_exports.string(),
    periodEnd: external_exports.string()
  }),
  projection: external_exports.object({
    storageGbEom: external_exports.number().nonnegative(),
    searchesEom: external_exports.number().int().nonnegative()
  })
});
var usagePeriod = external_exports.object({
  start: external_exports.string(),
  end: external_exports.string()
});

// ../shared/dist/plans.js
var planId = external_exports.enum(["trial", "free", "pro", "enterprise"]);
var catalogVersion = external_exports.number().int().positive();
var MB = 1e6;
var GB = 1e9;
var LIMITS_V1 = {
  // Trial: 14-day evaluation window (Phase 4). 1 GB is plenty to evaluate on; the prior
  // 10 GB was an over-generous default and was reverted.
  trial: {
    storageBytesIncluded: 1 * GB,
    searchesIncluded: 5e3,
    searchOverflow: "hard_stop",
    overageBudgetSearchesDefault: 0
  },
  free: {
    storageBytesIncluded: 512 * MB,
    searchesIncluded: 200,
    searchOverflow: "hard_stop",
    overageBudgetSearchesDefault: 0
  },
  pro: {
    storageBytesIncluded: 1 * GB,
    searchesIncluded: 1e4,
    searchOverflow: "overage",
    // $50 default overage budget / $0.005 per search ($5 per 1,000) = 10,000 searches.
    overageBudgetSearchesDefault: 1e4
  },
  // Enterprise is bespoke and invoiced off-platform (no gateway subscription); its caps
  // are not enforced on the automated path, so included amounts are effectively unlimited.
  enterprise: {
    storageBytesIncluded: Number.MAX_SAFE_INTEGER,
    searchesIncluded: Number.MAX_SAFE_INTEGER,
    searchOverflow: "overage",
    overageBudgetSearchesDefault: Number.MAX_SAFE_INTEGER
  }
};
var capLevel = external_exports.enum(["ok", "warn", "over"]);
var capNotice = external_exports.object({
  kind: external_exports.enum(["storage", "searches", "trial_ending", "suspended"]),
  level: capLevel
});
var capState = external_exports.object({
  status: tenantStatus,
  storage: external_exports.object({
    usedBytes: external_exports.number().nonnegative(),
  ...
  searches: external_exports.object({
    ...
    level: capLevel,
    overage: external_exports.boolean()
  }),
  trial: external_exports.object({ daysLeft: external_exports.number().int().nullable() }),
  notices: external_exports.array(capNotice)
});

// ../shared/dist/billing.js
var gatewayName = external_exports.enum(["stripe", "razorpay"]);
var gatewaySubscriptionStatus = external_exports.enum(["active", "past_due", "canceled"]);
var billingOutcome = external_exports.enum(["active", "past_due", "canceled", "reversal"]);
var subscription = external_exports.object({
  gateway: gatewayName,
  gatewayCustomerId: external_exports.string().min(1),
  gatewaySubscriptionId: external_exports.string().min(1),
  planId,
  catalogVersion,
  gatewayStatus: gatewaySubscriptionStatus,
  currentPeriodStart: external_exports.string().datetime(),
  currentPeriodEnd: external_exports.string().datetime(),
  gstin: external_exports.string().min(1).nullable()
});
var webhookEvent = external_exports.object({
  gateway: gatewayName,
  id: external_exports.string().min(1),
  type: external_exports.string().min(1),
  outcome: billingOutcome.nullable(),
  gatewayCustomerId: external_exports.string().min(1).nullable(),
  gatewaySubscriptionId: external_exports.string().min(1).nullable()
});
var planPickerOption = external_exports.object({
  planId,
  baseMinor: external_exports.number().int().nonnegative(),
  contactSales: external_exports.boolean()
});
var planPicker = external_exports.object({
  currency,
  catalogVersion,
  options: external_exports.array(planPickerOption)
});
