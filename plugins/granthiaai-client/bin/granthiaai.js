...
function chunkBytes(c) {
  return Buffer.byteLength(JSON.stringify(c));
}
function buildBatches(chunks) {
  ...
  for (const c of chunks) {
    const cost = chunkBytes(c);
    if (current.length > 0 && size + cost > INGEST_MAX_BATCH_BYTES) {
...
async function refreshUnderLock(auth, used, deps) {
  ...
  try {
    const current = await readCredentials() ?? used;
    if (current.refresh_token !== used.refresh_token) {
      return { kind: "ok", credentials: current };
    }
    if (!lock) {
      return { kind: "outage", message: "refresh lock busy" };
    }
...
async function syncSession(params) {
  ...
  if (outcome.kind === "unauthorized") {
    const refreshed = await refreshUnderLock(auth, credentials, deps);
